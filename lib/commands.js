module.exports = {
  ...
  clickNotFreeSeat: async function (page) {
    try {
      const selector = `.buying-scheme__chair.buying-scheme__chair_standart.buying-scheme__chair_taken:not(.buying-scheme__chair_selected)`;
      await page.waitForSelector(selector);
      await page.click(selector);
    } catch (error) {
      throw new Error(`NotFree seat is not clickable`);
    }
  },
